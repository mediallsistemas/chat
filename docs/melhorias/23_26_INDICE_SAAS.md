...
- [ ] 24.3 visão/status/progresso por unidade + **fan-out de boards** no attach + limpeza no detach
...
- [~] 24.5 **seed corrigido ✅** — `seed-strategic.ts` agora cria `PlanUnit` por plano (senão não apareceria em /processos pós-24.2) + `ensureTenantAndBackfill` no fim (tenant_id). Mantém os 6 planos ricos. **Deferido (de propósito):** consolidar p/ 1 plano→N unidades **depende de 24.3** (execução por unidade) — consolidar antes deixaria 5 unidades sem tasks (demo pobre). Remover `StrategicPlan.unitId` também fica p/ depois (muitos usos).
