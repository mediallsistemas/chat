...
import {
  ...
  useRemoveDependency,
  useKanbanBoard,
} from '@/features/kanban/hooks/use-kanban'
import { useTaskFiles } from '@/features/kanban/hooks/use-task-files'
...
export function TaskDetailModal({ open, onClose, taskId, unitId, boardId }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Detalhes da tarefa" size="lg">
      {isLoading ? (
        ...
        <div className="space-y-6">
          {/* Title + meta */}
          <div className="pb-5 border-b border-gs/40">
          ...
          <div className="pb-5 border-b border-gs/40">
            <div className="flex items-center justify-between gap-3 mb-4">
              <h4 className="text-xs font-semibold text-gray-700 uppercase tracking-wide flex items-center gap-1.5 min-w-0">
              ...
              {availableTasks.length > 0 && (
                <button
                  ...
                  className="text-xs text-gd hover:underline flex items-center gap-1 shrink-0"
                >
          ...
          <div className="pb-5 border-b border-gs/40">
            ...
            <div className="flex items-center gap-3 mt-4">
              <input
                ...
                className="flex-1 text-sm px-3 py-2 border border-gs rounded-xl focus:outline-none focus:border-gd focus:ring-1 focus:ring-gd/20 bg-page-bg"
                disabled={adding}
              />
              <button
                onClick={submitNewItem}
                disabled={!newItemText.trim() || adding}
                className="p-2 rounded-lg bg-gd text-white hover:opacity-90 disabled:opacity-40 transition-opacity shrink-0"
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-4">
              ...
              <button
                ...
                className="text-xs text-gd hover:underline flex items-center gap-1 disabled:opacity-50 shrink-0"
              >
