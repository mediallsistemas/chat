...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  private resolveException(exception: unknown): { status: number; message: string } {
    ...
    // Some libraries (csrf-csrf via http-errors) throw errors carrying a
    // recognizable `code`. Map known ones explicitly so they don't 500.
    if (exception && typeof exception === 'object' && (exception as any).code === 'EBADCSRFTOKEN') {
      return { status: HttpStatus.FORBIDDEN, message: 'Invalid or missing CSRF token' }
    }

    return { status: HttpStatus.INTERNAL_SERVER_ERROR, message: 'Internal server error' }
  }

  private isHttpErrorLike(exception: unknown): boolean {
