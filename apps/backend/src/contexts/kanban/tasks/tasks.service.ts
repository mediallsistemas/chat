...
export class TasksService {
  ...
  async create(unitId: string, dto: CreateTaskDto, user: JwtPayload) {
    ...
    this.eventBus.publish(
      new TaskCreatedEvent(task.id, task.title, unitId, dto.boardId, dto.responsibleUserId ?? null, user.sub),
    )
  ...
  async move(unitId: string, taskId: string, dto: MoveTaskDto, user: JwtPayload) {
    ...
    if (targetColumn?.isDoneColumn) {
      this.eventBus.publish(new TaskCompletedEvent(taskId, task.title, unitId, task.boardId, user.sub))
    }
