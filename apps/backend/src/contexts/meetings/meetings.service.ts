...
import { MeetingCancelledEvent } from './events/meeting-cancelled.event'
import { MeetingStartedEvent } from './events/meeting-started.event'
import { MeetingEndedEvent } from './events/meeting-ended.event'
import { RecordingConsentRequestedEvent } from './events/recording-consent-requested.event'
import { RecordingConsentUpdatedEvent } from './events/recording-consent-updated.event'
import { RecordingStartedEvent } from './events/recording-started.event'
import { RecordingStoppedEvent } from './events/recording-stopped.event'
...
export class MeetingsService {
  ...
  async create(unitId: string, userId: string, dto: CreateMeetingDto) {
    ...
    if (invitedIds.length > 0) {
      this.eventBus.publish(
        new MeetingScheduledEvent(meeting.id, meeting.title, unitId, meeting.creator.name, invitedIds),
      )
    }

    this.eventBus.publish(new MeetingCreatedEvent(unitId, meeting))
    return meeting
  }
  ...
  async update(unitId: string, meetingId: string, userId: string, dto: UpdateMeetingDto) {
    ...
    this.eventBus.publish(new MeetingUpdatedEvent(unitId, updated))
    return updated
  }
  ...
  async cancel(unitId: string, meetingId: string, userId: string) {
    ...
    this.eventBus.publish(new MeetingCancelledEvent(unitId, { meetingId }))
    return updated
  }
  ...
  async startRoom(unitId: string, meetingId: string, userId: string) {
    ...
    this.eventBus.publish(new MeetingStartedEvent(unitId, { meetingId, roomId: meeting.roomId }))
    return updated
  }
  ...
  async startRecording(unitId: string, meetingId: string, userId: string) {
    ...
    try {
      ...
      this.eventBus.publish(new RecordingStartedEvent(unitId, meetingId))
  ...
  async stopRecording(unitId: string, meetingId: string, userId: string) {
    ...
    this.eventBus.publish(new RecordingStoppedEvent(unitId, { meetingId, recordingUrl }))
    return { recordingUrl }
  }
