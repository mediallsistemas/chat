...
import { JwtPayload, TicketStatus, NotificationType } from '@mediall/types'
    ...
    if (dto.assignedTo && dto.assignedTo !== user.sub) {
      this.notifications
        .create({
          ...
          body: `O chamado "${dto.title}" foi atribuído a você.`,
          entityType: 'ticket',
          entityId: ticket.id,
        })
        .catch(() => undefined)
    ...
    if (wasAssigned && dto.assignedTo !== user.sub) {
      this.notifications
        .create({
          ...
          body: `O chamado "${updated.title}" foi atribuído a você.`,
          entityType: 'ticket',
          entityId: ticketId,
        })
        .catch(() => undefined)
    ...
    if (dto.status && dto.status !== ticket.status && ticket.reportedBy !== user.sub) {
      this.notifications
        .create({
          ...
          body: `O chamado "${updated.title}" mudou para "${dto.status}".`,
          entityType: 'ticket',
          entityId: ticketId,
        })
        .catch(() => undefined)
